...
export default {
  ...
  computed: {
    ...
    skuNumber() {
      return this.$route.query.skuNumber;
    },
    ...mapGetters(["cartList"]),
    cartInfoList() {
    ...
    totalPrice() {
      let sum = 0;
      let cartInfoList = this.cartInfoList.filter((cartInfo) => {
    ...
    handler: _.throttle(async function (type, num, cart) {
      ...
      switch (type) {
        case "add":
          ...
          break;
        case "sub":
          changeNum = cart.skuNum < 1 ? 0 : -1;
          break;
        case "input":
          if (isNaN(num) || num < 0) {
            changeNum = 0;
          } else {
            changeNum = num - cart.skuNum;
          }
          break;
      }
      try {
